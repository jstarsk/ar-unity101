    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_7480 = _Time + _TimeEditor;
                float CloudsPoleUV_ang = node_7480.g;
                ...
                float3 normalDirection = lerp(normalize(mul( unity_ObjectToWorld, float4((_Mask.r*normX( Rotate( (_Relief*NormalX.rgb*WaterRelief) , _RandomSeed ) , NormalsLocalNormalized ) + _Mask.g*normYX( Rotate2( (_Relief*NormalY.rgb*WaterRelief) , (_RandomSeed*4.0) ) , NormalsLocalNormalized ) + _Mask.b*normXZ( Rotate3( (_Relief*NormalZ.rgb*WaterRelief) , (_RandomSeed*8.0) ) , NormalsLocalNormalized )),0) ).xyz.rgb),i.normalDir,max(((1.0 - Freezing)*_water),_clouds));
...
////// Emissive:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 timeClouds = _Time + _TimeEditor;
                fixed PolesCaps = saturate((poles*-5.0+4.5));
                float4 node_8840 = _Time + _TimeEditor;
                float CloudsPoleUV_ang = node_8840.g;
                float CloudsPoleUV_spd = (_CloudsSpeed*(-0.1));
                float CloudsPoleUV_cos = cos(CloudsPoleUV_spd*CloudsPoleUV_ang);
                float CloudsPoleUV_sin = sin(CloudsPoleUV_spd*CloudsPoleUV_ang);
                float2 CloudsPoleUV_piv = half2(0.5,0.5);
