...
Shader "Human Unit/Planet Simple" {
    ...
    SubShader {
        ...
        Pass {
            ...
            half Clouds( half2 UV , half time , sampler2D tex , half speed , fixed E ){

            speed=speed*0.3;
            half1 sinTime = frac(sin(time.x*speed)*0.05);
            half1 sinTime2 = frac(sin(time.x*speed+0.78)*0.05);
            half1 sinTime3 = frac(sin(time.x*speed+1.57)*0.05);
            half1 sinTime4 = frac(sin(time.x*speed+2.35)*0.05);
            half1 clouds1 = tex2D(tex, half2(-UV.r+frac(speed*time.x*0.035+0.3), -UV.g+sinTime2)).r;
            half1 clouds2 = tex2D(tex, half2(UV.r+frac(speed*time.x*0.03),UV.g+sinTime)).r;
            half1 clouds3 = tex2D(tex, half2(-UV.r-frac(speed*time.x*0.035-0.35),-UV.g+sinTime3)).r;
            half1 clouds4 = tex2D(tex, half2(UV.r-frac(speed*time.x*0.03),UV.g+sinTime4)).r;
			return saturate(lerp(clouds1*clouds2+2*clouds3*clouds2*1.1,(2*clouds1+clouds2+2*clouds3+clouds2)*0.3,E));
            }
			half CloudsSimple( half2 UV , half time , sampler2D tex , half speed , fixed E ){
			#if _COMPLEXCLOUDS
			return(Clouds(UV,time,tex,speed,E));
			#endif
            speed=speed*0.3;
            half1 sinTime = frac(sin(time.x*speed)*0.05);
            half1 sinTime2 = frac(sin(time.x*speed+1.57)*0.05);
            half1 clouds1 = tex2D(tex, half2(-UV.r+frac(speed*time.x*0.035+0.3), -UV.g+sinTime2)).r;
            half1 clouds2 = tex2D(tex, half2(UV.r+frac(speed*time.x*0.03),UV.g+sinTime)).r;
            ...
            half CloudsShadows( half2 UV , half time , sampler2D tex , half speed , fixed E ){
            speed=speed*0.3;
            half1 sinTime = frac(sin(time.x*speed)*0.05);
            half1 sinTime2 = frac(sin(time.x*speed+0.78)*0.05);
            half1 sinTime3 = frac(sin(time.x*speed+1.57)*0.05);
            half1 sinTime4 = frac(sin(time.x*speed+2.35)*0.05);
            half1 clouds1 = tex2Dbias(tex, half4(-UV.r+frac(speed*time.x*0.035+0.3), -UV.g+sinTime2,0,3)).r;
            half1 clouds2 = tex2Dbias(tex, half4(UV.r+frac(speed*time.x*0.03),UV.g+sinTime,0,3)).r;
            half1 clouds3 = tex2Dbias(tex, half4(-UV.r-frac(speed*time.x*0.035-0.35),-UV.g+sinTime3,0,3)).r;
            half1 clouds4 = tex2Dbias(tex, half4(UV.r-frac(speed*time.x*0.03),UV.g+sinTime4,0,3)).r;
            return saturate(lerp(clouds1*clouds2+2*clouds3*clouds2*1.1,(2*clouds1+clouds2+2*clouds3+clouds2)*0.3,E));
            }
			half CloudsShadowsSimple( half2 UV , half time , sampler2D tex , half speed , fixed E ){
			#if _COMPLEXCLOUDS
            ...
            VertexOutput vert (VertexInput v) {
				...
				half3 PosScaled = ((_Size*4.9+0.1)*mul( unity_WorldToObject, half4((o.posWorld.rgb-objPos.rgb),0) ).xyz);

				half UVRotX_ang = _RandomSeed;
                half UVRotX_cos = cos(UVRotX_ang);
                half UVRotX_sin = sin(UVRotX_ang);
                half2 UVRotX_piv = half2(0.5,0.5);
                half2 UVRotX = (mul((PosScaled.gb*0.5+0.5)-UVRotX_piv,half2x2( UVRotX_cos, -UVRotX_sin, UVRotX_sin, UVRotX_cos))+UVRotX_piv);
                half2 UVPanX = (UVRotX+_RandomSeed*half2(0.7,0));
                half2 UVX = lerp(UVPanX,(half2(0.3,0)+UVPanX),(sign(v.normal.r)*0.5+0.5));

				half UVRotY_ang = (_RandomSeed*4.0);
                half UVRotY_cos = cos(UVRotY_ang);
                half UVRotY_sin = sin(UVRotY_ang);
                half2 UVRotY_piv = half2(0.5,0.5);
                half2 UVRotY = (mul((PosScaled.rb*0.5+0.5)-UVRotY_piv,half2x2( UVRotY_cos, -UVRotY_sin, UVRotY_sin, UVRotY_cos))+UVRotY_piv);
                half2 UVPanY = (UVRotY+_RandomSeed*half2(0,0.7));
                half2 UVY = lerp(UVPanY,(half2(0.3,0.3)+UVPanY),(sign(v.normal.g)*0.5+0.5));

				o.uvxy=half4(UVX,UVY);

				half UVRotZ_ang = (_RandomSeed*8.0);
                half UVRotZ_cos = cos(UVRotZ_ang);
                half UVRotZ_sin = sin(UVRotZ_ang);
                half2 UVRotZ_piv = half2(0.5,0.5);
                half2 UVRotZ = (mul((PosScaled.rg*0.5+0.5)-UVRotZ_piv,half2x2( UVRotZ_cos, -UVRotZ_sin, UVRotZ_sin, UVRotZ_cos))+UVRotZ_piv);
                half2 UVPanZ = (UVRotZ+_RandomSeed*half2(-0.7,0.7));
                half2 UVZ = lerp(UVPanZ,(half2(0,0.3)+UVPanZ),(sign(v.normal.b)*0.5+0.5));

				o.uvz=half4(UVZ,(PosScaled.rb*0.5+0.5));
            ...
            half4 frag(VertexOutput i) : COLOR {
				...
				half2 _UVX = i.uvxy.xy;
				half2 _UVY = i.uvxy.zw;
				half2 _UVZ = i.uvz.xy;
				half2 _PolesPos1 = i.uvz.zw;
                ...
                half4 timeClouds = _Time + _TimeEditor;
                fixed PolesCaps = saturate((poles*-5.0+4.5));
                half4 node_1988 = _Time + _TimeEditor;
                half CloudsPoleUV_ang = node_1988.g;
                half CloudsPoleUV_spd = (_CloudsSpeed*(-0.1));
                half CloudsPoleUV_cos = cos(CloudsPoleUV_spd*CloudsPoleUV_ang);
                half CloudsPoleUV_sin = sin(CloudsPoleUV_spd*CloudsPoleUV_ang);
                half2 CloudsPoleUV_piv = half2(0.5,0.5);
                half2 CloudsPoleUV = (mul((_PolesPos1*0.6+0.21)-CloudsPoleUV_piv,half2x2( CloudsPoleUV_cos, -CloudsPoleUV_sin, CloudsPoleUV_sin, CloudsPoleUV_cos))+CloudsPoleUV_piv);
				
                half4 _CloudsPoleDif = tex2D(_CloudsPole,TRANSFORM_TEX(CloudsPoleUV, _CloudsPole)).a;
                half _clouds = pow(saturate((((CloudsAmounbtNeg+CloudsSimple( i.uv0 , timeClouds.g , _Clouds , _CloudsSpeed , _CloudsAvgBlend ))*PolesCaps)+(CloudsAmounbtNeg+saturate((_CloudsPoleDif-PolesCaps))))),(_CloudsSpread*-0.5+1.0));
                #else
				half _clouds=0;
				#endif
...
////// Emissive:
                ...
                half PlantsMask = saturate(((1.0 - saturate((((saturate((PolesReachMao-(-1*(1.0 - (_VegetationFrostResistance*0.5+0.5)))))*1.3+-1.0)+poles+(0.35*heightNormalized))*2.857143+-0.857143)))*(0 + ( (((1.0 - saturate((0 + ( (Heat - _VegetationContrast) * (1.0 - 0) ) / ((1.0 - _VegetationContrast) - _VegetationContrast))))*((_Fertility*1.6+-1.0)+((_Mask.r*VegetationX.a + _Mask.g*VegetationY.a + _Mask.b*VegetationZ.a)+(heightNormalized*(-0.3))))) - _VegetationContrast) * (1.0 - 0) ) / ((1.0 - _VegetationContrast) - _VegetationContrast))));
